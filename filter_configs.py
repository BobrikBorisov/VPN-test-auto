    ...
    try:
        with open(filename, 'r') as f:
            for line in f:
                ...
                if any(x in domain for x in ['yandex', 'kinopoisk', 'ya.ru']): DOMAINS["YANDEX"].append(domain)
                elif any(x in domain for x in ['mail.ru', 'vk.com', 'ok.ru']): DOMAINS["VK"].append(domain)
...
def verify_and_qualify(line):
    ...
    if provider == "FOREIGN":
        if "flow=xtls-rprx-vision" in line:
            ...
            return None # Отбрасываем зарубежные Reality без Vision
    else: # RU_YANDEX или RU_VK
        quality = 2 # Российские серверы тоже важны
        
    # --- Проверка стабильности ---
    if is_server_stable(ip, port):
        print(f"[STABLE] Найден стабильный сервер: {ip}:{port} (Тип: {provider})")
        return line, quality
        
    print(f"[UNSTABLE] Сервер {ip}:{port} не прошел проверку стабильности.")
    return None, None
...
def main():
    ...
    print(f"\n>>> Готово! Свежий и стабильный улов: {len(final_configs)} конфигов.")
